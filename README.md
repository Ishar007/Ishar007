<div align="center">

# 👋 Hi, I'm Ishar Roy

<p align="center">
<a href="https://github.com/Ishar007"><img src="https://img.shields.io/badge/GitHub-Ishar007-181717?style=for-the-badge&logo=github"></a>
<a href="https://www.linkedin.com/in/ishar-roy/"><img src="https://img.shields.io/badge/LinkedIn-Connect-blue?style=for-the-badge&logo=linkedin"></a>
<a href="mailto:isharroy8@gmail.com"><img src="https://img.shields.io/badge/Email-Contact-red?style=for-the-badge&logo=gmail"></a>
</p>

![](https://komarev.com/ghpvc/?username=Ishar007&style=for-the-badge&color=blueviolet)

### 💻 Full-Stack Developer | AI Enthusiast | B.Tech CSE Student

Passionate about building scalable, real-world software solutions using modern web technologies.

<img src="https://readme-typing-svg.herokuapp.com?font=Poppins&size=24&duration=3000&pause=1000&color=7F5AF0&center=true&vCenter=true&width=700&lines=Full-Stack+Developer;React+%7C+Node.js+%7C+Express;MongoDB+%7C+MySQL+%7C+Firebase;AI+Enthusiast;Building+Real-World+Applications;Open+to+Software+Engineering+Opportunities" />

</div>

---

# 🚀 About Me

🎓 **B.Tech in Computer Science & Engineering**

🏫 Future Institute of Engineering and Management

📍 India

💡 I enjoy designing and building full-stack web applications that solve practical business problems. I'm passionate about learning modern technologies, exploring AI, and continuously improving my software engineering skills.

> **"The best way to learn software engineering is by building real-world projects."**

---

# 🌱 Currently Learning

- 🤖 Artificial Intelligence
- ⚛️ Advanced React
- 🔥 Firebase
- ☁️ Cloud Computing
- 📍 Geolocation Applications
- 📚 Data Structures & Algorithms

---

# 💻 Tech Stack

| Category | Technologies |
|-----------|--------------|
| **Frontend** | React, JavaScript, HTML5, CSS3 |
| **Backend** | Node.js, Express.js |
| **Database** | MongoDB, MySQL, Firebase Firestore |
| **Tools** | Git, GitHub, VS Code |

---

# 🌟 Featured Projects

## 🚚 Outbound Logistics Tracking System

A full-stack logistics tracking platform featuring real-time shipment monitoring, customer order management, delivery tracking, and interactive maps.

**Tech Stack**

React • Node.js • Express • MongoDB • Leaflet

🔗 **Repository:** https://github.com/Ishar007/outbound-logistics-tracking-system

---

## 🚪 Visitor Gate Pass Management System

A digital visitor management solution for secure visitor registration, gate pass generation, and streamlined entry management.

**Tech Stack**

React • Node.js • Express • MySQL

🔗 **Repository:** https://github.com/Ishar007/visitor-gate-pass-management-system

---

## 📍 Geo-Location Attendance Tracking System

A smart attendance system that verifies employee location using GPS and geofencing before recording attendance. Includes Firebase Authentication, Firestore integration, and an analytics dashboard.

**Tech Stack**

React • Firebase • Firestore • JavaScript

🔗 **Repository:** https://github.com/Ishar007/geo-location-attendance-tracking-system

---

## 🌐 Page Pulse

A production-ready REST API for website monitoring and URL auditing with automated testing and GitHub Actions.

**Tech Stack**

Node.js • Express • Jest • GitHub Actions

🔗 **Repository:** https://github.com/Ishar007/page-pulse

---

# 🏆 GitHub Trophies

<p align="center">
<img src="https://github-profile-trophy.vercel.app/?username=Ishar007&theme=tokyonight&no-frame=true&no-bg=true&margin-w=15&row=1&column=6" />
</p>

---

# 📊 GitHub Statistics

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=Ishar007&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true"/>

<img height="180em" src="https://github-readme-streak-stats.herokuapp.com?user=Ishar007&theme=tokyonight"/>

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Ishar007&layout=compact&theme=tokyonight"/>

</div>

---

# 📈 Contribution Graph

<div align="center">

![](https://github-readme-activity-graph.vercel.app/graph?username=Ishar007&theme=tokyo-night)

</div>

---

# 🎯 Current Goals

- 🚀 Build more production-ready Full-Stack projects
- 🤖 Develop AI-powered applications
- 📚 Strengthen Data Structures & Algorithms
- ☁️ Learn Cloud Technologies
- 💼 Secure a Software Engineering role

---

# 📫 Connect With Me

📧 **Email:** **isharroy8@gmail.com**

💼 **LinkedIn:** https://www.linkedin.com/in/ishar-roy/

🌐 **GitHub:** https://github.com/Ishar007

---

<div align="center">

### ⭐ Thanks for visiting my profile!

*"Building software, solving problems, and learning something new every day."*

</div>
